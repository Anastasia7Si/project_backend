...
async def get_dealers_prices(db: AsyncSession, dealer_name: str = None, markup: bool = None,  limit: int = None):
    if dealer_name:
        results = await db.execute(
            select(
                models.DealerPrice
...
async def create_relation_products(db: AsyncSession, product_id: int, product_key: schemas.ProductDealerKeyCreate):
    product_relation = models.ProductDealerKey(key=product_id, **product_key.model_dump())
